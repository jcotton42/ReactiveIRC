...
namespace Tests {
    public class IRCMessageTests {
        ...
        public void TestMessageParsing(MessageTest data) {
            ...
            Assert.Equal(atoms.Params ?? (IEnumerable<string>)ImmutableArray<string>.Empty, message.Parameters);
            Assert.Equal(atoms.Tags ?? (IDictionary<string, string>)ImmutableDictionary<string, string>.Empty, message.Tags, new NullFriendlyDictionaryComparer());
        }
    ...
    public class NullFriendlyDictionaryComparer : IEqualityComparer<KeyValuePair<string, string>> {
